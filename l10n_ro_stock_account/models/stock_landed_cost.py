...
class StockLandedCost(models.Model):
    ...
    def l10n_ro_distribute_cost_propagate(
        self, cost_to_add_byproduct, line, cost_to_add=0, propagate=False, svls=None
    ):
        # import ipdb; ipdb.set_trace()
        if not svls and line:
            svls = line.move_id.stock_valuation_layer_ids
        if not line:
            line = self.env["stock.valuation.adjustment.lines"]
        for svl in svls.filtered(lambda s: s.quantity != 0):
            ...
            cost_to_add_byproduct, svl_cost_to_add = self.l10n_ro_distribute_cost(
                cost_to_add_byproduct, cost_to_add, line, propagate, svl
            )
            self.l10n_ro_propagate_cost_out(
                cost_to_add_byproduct, svl_cost_to_add, line, svl
            )
            if svl.quantity > 0 and svl.l10n_ro_svl_dest_ids:
                distributed_svls = svl.l10n_ro_svl_dest_ids.mapped(
                    "stock_move_id.stock_valuation_layer_ids"
                )
                _logger.info("propagate")
                _logger.info(svl.l10n_ro_svl_dest_ids)
                self.l10n_ro_distribute_cost_propagate(
                    cost_to_add_byproduct,
                    self.env["stock.valuation.adjustment.lines"],
                    cost_to_add=svl_cost_to_add,
                    propagate=True,
                    svls=distributed_svls,
                )
        return True

    def l10n_ro_distribute_cost(
        self, cost_to_add_byproduct, cost_to_add, line, svl, propagate=False
    ):
        ...
        valuation_layer = self.l10n_ro_create_valuation_layer(line, svl, cost_to_add)
        ...
        if product.valuation == "real_time":
            ...
            amls = line._l10n_ro_prepare_accounting_entries(
                valuation_layer, svl_move_vals, svl_cost_to_add, svl_type="in"
            )
            if amls:
    ...
    def l10n_ro_propagate_cost_out(self, cost_to_add_byproduct, cost_to_add, line, svl):
        move_obj = self.env["account.move"]
        valuation_layer_ids = []
        move_vals = self.l10n_ro_get_move_vals(self)
        product = svl.product_id
        ...
        for cost in self:
            ...
            for line in cost.valuation_adjustment_lines.filtered(
                ...
                cost.l10n_ro_distribute_cost_propagate(
                    cost_to_add_byproduct, line, cost_to_add=line.additional_landed_cost
                )
