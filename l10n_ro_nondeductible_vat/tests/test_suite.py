...
class TestNonDeductibleVAT(TestNondeductibleCommon):
    """Non deductible VAT on invoices, inventories, consumptions and VAT on
    payment.

    Odoo builds a new company and reloads the whole Romanian chart of accounts
    for each ``post_install`` test class, see
    ``AccountTestInvoicingCommon.setup_independent_company``, so these
    scenarios share a single class rather than one per file.
    """
