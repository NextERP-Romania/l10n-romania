...
class AccountTaxExtend(models.Model):
    ...
    l10n_ro_nondeductible_tax_id = fields.Many2one("account.tax", copy=False)
    l10n_ro_is_nondeductible = fields.Boolean(
        compute="_compute_boolean_l10n_ro_nondeductible", store=True
    )

    @api.depends("invoice_repartition_line_ids", "refund_repartition_line_ids")
    def _compute_boolean_l10n_ro_nondeductible(self):
        ro_recs = self.filtered("is_l10n_ro_record")
        for record in ro_recs:
...
class AccountTaxRepartitionLineExtend(models.Model):
    ...
    l10n_ro_nondeductible = fields.Boolean()
    l10n_ro_exclude_from_stock = fields.Boolean()
